# 流光智能数据资产平台 - 产品需求文档 (PRD V5)

> 版本：V5 | 更新日期：2026-03-11 | 状态：已实现
...
**流光 (Liuguang)** 是一款面向企业的智能数据资产管理平台。它通过 LLM 驱动的 ETL 管道，将员工散落在飞书（Lark）多维表格、云文档、聊天记录和会议纪要中的隐性知识，自动抽取、清洗、结构化，沉淀为可检索、可分析、可共享的企业数据资产。平台内置知识图谱构建、混合 RAG 智能问答、报告生成、领导力分析等 AI 能力，为每位员工提供个人数据驾驶舱和智能办公助手。

### 1.2 核心业务闭环

```
数据采集 → 智能清洗 → 资产沉淀 → 知识图谱 → 智能应用
```

1. **隐性知识资产化**：通过 ETL 引擎 + 本地上传 + 云文档同步，将多源数据自动抓取、LLM 清洗并沉淀为标准化资产。
2. **安全隔离访问**：飞书 SSO 身份绑定 + 行级数据权限（个人数据 + 直接共享 + 部门共享），管理员全局可见。
3. **知识图谱构建**：从内容中自动提取实体与关系，构建可视化知识网络，支持社区发现与洞察生成。
4. **智能应用层**：基于结构化资产提供 RAG 问答、报告生成、领导力分析、日程管家、智能待办等 AI 服务。

### 1.3 目标用户

| 角色 | 权限 | 核心场景 |
|------|------|----------|
| 普通员工 (employee) | 仅看自己的数据 + 被共享的数据 | 数据导入、智能问答、查看看板、管理待办 |
...
┌──────────────────────────▼──────────────────────────────────────┐
│                    后端 (FastAPI:8000)                           │
│  24 个 API Router │ ~110 个端点 │ SSE 流式输出                    │
│  ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐           │
│  │ Auth/RBAC│ │ ETL管道  │ │ RAG引擎  │ │ KG构建器 │           │
│  └──────────┘ └──────────┘ └──────────┘ └──────────┘           │
│  ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐           │
│  │ 报告生成 │ │ 领导力   │ │ 日程管家 │ │ 待办提取 │           │
│  └──────────┘ └──────────┘ └──────────┘ └──────────┘           │
...
| 沟通记录 | Communication | 会议/聊天/录音 | comm_type, participants, agenda, conclusions, action_items, duration |
...
#### 3.6.4 内容-实体关联 (ContentEntityLink)
- 每条内容自动关联被提及的实体
- 支持双向查询：内容→实体、实体→内容
...
- **查询改写**：LLM 优化用户口语化输入
- **双路检索**：
  - Vector 余弦相似度（pgvector）
  - BM25 关键词检索
- **结果融合**：Reciprocal Rank Fusion (RRF)
- **Graph-RAG 增强**：从查询中提取实体，遍历知识图谱获取补充上下文
- **标签匹配**：自动识别查询中的相关标签
- **权限过滤**：所有检索结果均经过 RLS 过滤

#### 3.7.2 对话功能
- **SSE 流式输出**：实时返回推理过程 + 回答内容 + 来源引用
- **会话管理**：创建/列表/详情/删除/导出（Markdown）
- **来源引用**：回答中包含可点击的来源链接，跳转至原始内容页
- **附件解析**：上传文件直接作为对话上下文
- **快捷模板**：本周摘要、项目进展、会议待办、人员协作
...
- **数据范围选择**：全部数据 / 自定义日期区间 / 指定实体
- **SSE 流式生成**：实时渲染 Markdown 报告内容
- **读者指定**：选择知识图谱中的人物实体作为目标读者
- **发布到飞书**：一键推送为飞书云文档
- **报告管理**：查看/编辑/删除已生成报告

### 3.9 数据提取与清洗规则

#### 3.9.1 提取规则 (ExtractionRule)

用于非结构化数据（文档/沟通记录）的关键信息自动提取。
...
#### 3.12.1 用户级共享 (UserVisibilityOverride)
- 直接将个人数据授权给指定用户

#### 3.12.2 部门级共享 (UserDeptSharing)
- 将个人数据授权给整个部门
- 部门树形结构（可搜索）
...
**数据库迁移**：46 个版本，覆盖完整的 Schema 演进历程。
