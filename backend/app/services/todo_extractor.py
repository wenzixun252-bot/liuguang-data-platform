...
def _name_match(user_name: str, text: str) -> bool:
    """精确或部分中文名匹配。支持全名匹配和姓名后两字匹配（如"梓旬"匹配"文梓旬"）。"""
    if not user_name or not text:
        return False
    # 全名精确匹配
    if user_name == text:
        return True
    # 用户名出现在 text 中（如 assignee 字段为 "文梓旬, 杨湘"）
    if user_name in text:
        return True
    # 支持部分名匹配：text 中包含用户名的后两字（如"梓旬"匹配"文梓旬"）
    if len(user_name) >= 3:
        short_name = user_name[1:]  # 去掉姓
...
## 判断规则
1. **聊天消息中的请求/委托**：只有当别人**直接对「{user_name}」说话**（提到其姓名/部分姓名、用"你"指代且上下文明确指向「{user_name}」）并要求做某事时，才算是分配给「{user_name}」的待办。例如"梓旬帮我看下XX"、"@{user_name} 请你这边帮走一下流程"
2. **会议纪要中的任务分配**：如"{user_name}负责XX"、"@{user_name} 请完成XX"
3. 如果用户名的一部分出现在文本中（如全名为"文梓旬"而消息中写"梓旬"），同样视为指向该用户
4. **群聊中他人之间的对话，即使「{user_name}」也在群里，如果消息没有指名道姓地提到「{user_name}」，则与「{user_name}」无关，不要提取**。仅当「{user_name}」是消息的发送者且消息中包含自我承诺（如"我来做XX"、"我跟进"）时，才视为自己的待办
5. **任务描述中提到其他人名作为执行者的，属于他人任务，完全忽略**。例如"安排杨湘帮助部署"是杨湘的任务；"让张三跟进"是张三的任务。即使没有明确标注"负责人"，只要能从上下文判断出实际执行者是其他人，就不要提取
    ...
    for comm in comms:
        ...
        if comm.comm_type in ("meeting", "recording") and comm.action_items:
            for item in comm.action_items:
                ...
                # 用精确匹配或部分名称匹配（如"梓旬"匹配"文梓旬"）
                is_mine = user_name and _name_match(user_name, assignee)
                is_organizer_unassigned = (
                    not assignee and comm.initiator and _name_match(user_name, comm.initiator)
                )
